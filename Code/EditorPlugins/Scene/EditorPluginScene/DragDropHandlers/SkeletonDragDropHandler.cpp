...
void ezSkeletonComponentDragDropHandler::OnDragBegin(const ezDragDropInfo* pInfo)
{
  ...
  if (pInfo->m_sTargetContext == "viewport")
    CreateDropObject(pInfo->m_vDropPosition, "ezVisualizeSkeletonComponent", "Skeleton", GetAssetGuidString(pInfo), ezUuid(), -1);
  else
    CreateDropObject(pInfo->m_vDropPosition, "ezVisualizeSkeletonComponent", "Skeleton", GetAssetGuidString(pInfo), pInfo->m_TargetObject, pInfo->m_iTargetObjectInsertChildIndex);
